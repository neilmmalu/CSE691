...
bool Factory::pushBuffer(vector<int> loadOrder) {
...
void Factory::PartWorker(int i){
    ...
    wait:
        PART_SLEEPERS++;
        if (cv1.wait_until(lock, start + chrono::microseconds(timeOut), [this, loadOrder, &waits] { 
...
void Factory::ProductWorker(int i){
    int iteration = 0;
    while(iteration < NUM_ITER){
    ...
    wait:
        PROD_SLEEPERS++;
        if (cv2.wait_until(lock, start + chrono::microseconds(timeOut), [this, pickupOrder, &waits] { 
...
void part(Factory &F, int i){
    F.PartWorker(i);
}

void prod(Factory &F, int i){
    F.ProductWorker(i);
}
...
int main(){
    ...
    thread prodW[n];
    for (int i = 0; i < n; i++){
        partW[i] = thread(part, ref(F), i);
        prodW[i] = thread(prod, ref(F), i);
    }
    ...
    cout << "PRODUCTS COMPLETED: " << F.getTotalProd() << endl;
    cout << "Finish!" << endl;
    return 0;
}